...
from .classes import StoreReport, UtcEpochRange
from .config_keeper import ConfigKeeper
from .repos import *
from .utils import DateUtils, WeekDay, SubReportType
...
def calculateUpDownHours(utcEpochRangesInput:list[tuple[int,int]], sortedPolledStats:list[PolledStat], ignoreTill:int, pivotUtcEpoch:int):
    ...
    for idx in range(len(utcEpochRangesInput)):
...
class ReportService:
    @staticmethod
    def generate(reportStat:ReportStat):
        try:
            print("-------2nd function call init----------")
            reportFilePath = f"./reports/report-{reportStat.id}-v{reportStat.version}.csv"
            print(reportFilePath)
            # already existing : no effect
            if osPathExists(reportFilePath):
                return
            
            pivotEpoch = reportStat.runAt
            ...
            print("---------inside 2nd function call end --------")
    ...
    def getSameTzStoresUpDownHours(subReportTiming:UtcEpochRange, stores:list[StoreInfo], pivotUtcEpoch:int):
        ...
        concernedWeekDays: list[WeekDay] = (
            [subReportTiming.lWeekDay]
            if subReportTiming.lWeekDay is not None
            else list(WeekDay)
        )
